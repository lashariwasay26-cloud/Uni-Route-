...
async function seedChapter4() {
  console.log('🚀 Starting SAT Writing Chapter 4 Database Seeding...');
  ...
  for (const block of SAT_WRITING_CHAPTER_4_FULL.theoryBlocks) {
  ...
  for (const block of SAT_WRITING_CHAPTER_4_FULL.exerciseBlocks) {
  ...
  try {
    ...
    await supabase.from('sat_writing_ch4_theory').delete().neq('id', 'placeholder_dummy_key');
    ...
    for (let i = 0; i < theoryRows.length; i += theoryChunkSize) {
      ...
      const { error } = await supabase.from('sat_writing_ch4_theory').insert(chunk);
      if (error) {
    ...
    await supabase.from('sat_writing_ch4_exercises').delete().neq('id', 'placeholder_dummy_key');
    ...
    for (let i = 0; i < exerciseRows.length; i += exerciseChunkSize) {
      ...
      const { error } = await supabase.from('sat_writing_ch4_exercises').insert(chunk);
      if (error) {
    ...
    console.log('\n🎉 ALL CHAPTER 4 DATA SUCCESSFULLY SEEDED TO SUPABASE! 🎉');
