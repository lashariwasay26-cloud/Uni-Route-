...
async function seedChapter5() {
  console.log('🚀 Starting SAT Writing Chapter 5 Database Seeding...');
  ...
  for (const block of SAT_WRITING_CHAPTER_5_FULL.theoryBlocks) {
  ...
  for (const block of SAT_WRITING_CHAPTER_5_FULL.exerciseBlocks) {
  ...
  try {
    ...
    await supabase.from('sat_writing_ch5_theory').delete().neq('id', 'placeholder_dummy_key');
    ...
    for (let i = 0; i < theoryRows.length; i += theoryChunkSize) {
      ...
      const { error } = await supabase.from('sat_writing_ch5_theory').insert(chunk);
      if (error) {
    ...
    await supabase.from('sat_writing_ch5_exercises').delete().neq('id', 'placeholder_dummy_key');
    ...
    for (let i = 0; i < exerciseRows.length; i += exerciseChunkSize) {
      ...
      const { error } = await supabase.from('sat_writing_ch5_exercises').insert(chunk);
      if (error) {
    ...
    console.log('\n🎉 ALL CHAPTER 5 DATA SUCCESSFULLY SEEDED TO SUPABASE! 🎉');
