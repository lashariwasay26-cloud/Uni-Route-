...
async function seedChapter6() {
  console.log('🚀 Starting SAT Writing Chapter 6 Database Seeding...');
  ...
  for (const block of SAT_WRITING_CHAPTER_6_FULL.theoryBlocks) {
  ...
  for (const block of SAT_WRITING_CHAPTER_6_FULL.exerciseBlocks) {
  ...
  try {
    ...
    await supabase.from('sat_writing_ch6_theory').delete().neq('id', 'placeholder_dummy_key');
    ...
    for (let i = 0; i < theoryRows.length; i += theoryChunkSize) {
      ...
      const { error } = await supabase.from('sat_writing_ch6_theory').insert(chunk);
      if (error) {
    ...
    await supabase.from('sat_writing_ch6_exercises').delete().neq('id', 'placeholder_dummy_key');
    ...
    for (let i = 0; i < exerciseRows.length; i += exerciseChunkSize) {
      ...
      const { error } = await supabase.from('sat_writing_ch6_exercises').insert(chunk);
      if (error) {
    ...
    console.log('\n🎉 ALL CHAPTER 6 DATA SUCCESSFULLY SEEDED TO SUPABASE! 🎉');
