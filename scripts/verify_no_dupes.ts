...
for (const [key, items] of optMap.entries()) {
  if (items.length > 1) {
    remainingDuplicates++;
    console.log(`\nRemaining Duplicate Group ${remainingDuplicates} (${items.length} questions): [${key}]`);
    items.forEach(it => {
      console.log(`  - [${it.section}] ID: ${it.id} (Ans: ${it.q.correctAnswer ?? it.q.correctIndex})`);
    });
  }
}

console.log(`\nRemaining Duplicate Option Sets: ${remainingDuplicates}`);
