...
// Retrieve Supabase credentials from standard environment variables
...
async function seedChapter2() {
  console.log('🚀 Starting SAT Writing Chapter 2 Database Seeding...');
  ...
  for (const block of SAT_WRITING_CHAPTER_2_FULL.theoryBlocks) {
  ...
  for (const block of SAT_WRITING_CHAPTER_2_FULL.exerciseBlocks) {
  ...
  // 3. Uploading rows to Supabase in chunks
  ...
  try {
    ...
    // Clear existing to prevent duplicate errors if run multiple times
    await supabase.from('sat_writing_ch2_theory').delete().neq('id', 'placeholder');
    ...
    for (let i = 0; i < theoryRows.length; i += theoryChunkSize) {
      ...
      const { error } = await supabase.from('sat_writing_ch2_theory').insert(chunk);
      if (error) {
    ...
    // Clear existing to prevent duplicate errors
    await supabase.from('sat_writing_ch2_exercises').delete().neq('id', 'placeholder');
    ...
    for (let i = 0; i < exerciseRows.length; i += exerciseChunkSize) {
      ...
      const { error } = await supabase.from('sat_writing_ch2_exercises').insert(chunk);
      if (error) {
    ...
    console.log('\n🎉 ALL CHAPTER 2 DATA SUCCESSFULLY SEEDED TO SUPABASE! 🎉');

  } catch (err: any) {
    console.error('\n❌ Seeding failed with error:', err.message || err);
    console.log('\n💡 Troubleshooting Checklists:');
    console.log('1. Have you run the SQL DDL commands in your Supabase Dashboard SQL Editor?');
    console.log('2. Make sure RLS is disabled or an insert policy is temporarily created during seeding.');
  }
}
