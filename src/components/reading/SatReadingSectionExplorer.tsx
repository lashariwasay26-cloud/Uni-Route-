...
import { 
  ...
  fetchSatReadingChapter2FromSupabase,
  SatReadingChapter2Data,
  fetchSatReadingChapter3FromSupabase,
  SatReadingChapter3Data,
  fetchSatReadingChapter4FromSupabase,
  SatReadingChapter4Data,
...
interface SatReadingSectionExplorerProps {
...
export const SatReadingSectionExplorer: React.FC<SatReadingSectionExplorerProps> = ({ user, onOpenAuth }) => {
  ...
  // Restore scroll position when leaving a chapter profile back to reading chapters list
  useEffect(() => {
    ...
    <div id="sat-reading-section-explorer" className="space-y-6 text-slate-900 min-h-[600px] pb-12 relative overflow-x-hidden w-full">
      {!selectedChapterId ? (
        ...
        >
          ...
          <motion.div
            key={`reading-chapter-${selectedChapterId}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.15 }}
                    ...
                    >
                      ...
                      {(q.passageText || q.passage?.text) && (
                        <div className="p-3.5 sm:p-5 rounded-xl sm:rounded-2xl bg-white/95 border border-slate-200/90 font-serif text-xs sm:text-[13.5px] text-slate-800 leading-relaxed shadow-2xs">
                          ...
                          <FormattedPassageText text={q.passageText || q.passage?.text || ''} />
                        </div>
                      )}
