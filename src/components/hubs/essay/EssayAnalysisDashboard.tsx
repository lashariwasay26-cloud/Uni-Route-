...
export const EssayAnalysisDashboard: React.FC<EssayAnalysisDashboardProps> = ({
  ...
  return (
    <div id="essay-analysis-dashboard" className="space-y-4 sm:space-y-6 animate-in fade-in-50 max-w-5xl mx-auto">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="grid grid-cols-2 gap-2 w-full sm:w-auto sm:flex sm:items-center sm:gap-2">
          <button
            ...
            className="h-10 px-3 sm:px-4 rounded-xl border border-slate-200 bg-white text-slate-700 text-xs font-bold transition-colors flex items-center justify-center gap-1.5 cursor-pointer hover:bg-slate-50 w-full sm:w-auto"
          >
          ...
          <button
            ...
            onClick={onAnalyze}
            className="h-10 px-3 sm:px-5 rounded-xl bg-slate-900 hover:bg-black disabled:opacity-50 text-white font-bold text-xs shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer w-full sm:w-auto"
          >
            {isAnalyzing ? (
              <>
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
          <div className="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200/80">
            <span className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block">Word Count</span>
            <span className={`text-lg sm:text-xl font-black ${wordCount > wordLimit ? 'text-amber-600' : 'text-slate-950'}`}>
              {wordCount} <span className="text-[11px] font-normal text-slate-400">/ {wordLimit}</span>
            </span>
          </div>

          <div className="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200/80">
            <span className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block">Paragraphs</span>
          ...
          <div className="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200/80">
            <span className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block mb-0.5">Prompt Alignment</span>
            <div className="mt-1">
          ...
          <div className="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200/80">
            <span className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block">Overall Status</span>
      ...
      {/* LOADING STATE */}
      {isAnalyzing && (
        <div className="py-12 text-center space-y-4 bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
          <RefreshCw className="w-8 h-8 text-indigo-600 animate-spin mx-auto" />
          <div>
            <h3 className="text-base sm:text-lg font-bold text-slate-950">Analyzing your essay...</h3>
      ...
      {analysis && !isAnalyzing && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            {/* Strengths Card */}
            <div className="md:col-span-5 bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] space-y-3">
              ...
              <ul className="space-y-2 text-xs text-slate-700">
                {(analysis.mainStrengths || analysis.strengths || []).map((str, idx) => (
                  <li key={idx} className="flex items-start gap-2 bg-emerald-50/80 p-2.5 rounded-xl border border-emerald-100">
            ...
            <div className="md:col-span-7 bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] space-y-3">
              <h3 className="text-sm font-bold text-slate-950 flex items-center gap-2">
              ...
              <div className="space-y-2.5">
                {(analysis.priorityImprovements || []).map((item, idx) => (
                  <div
                    key={item.id || idx}
                    className="p-3 sm:p-4 rounded-xl bg-amber-50/60 border border-amber-200/80 space-y-1.5 text-xs"
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button
                        onClick={onBackToDraft}
                        className="text-[9px] font-bold px-2 py-0.5 rounded-lg bg-amber-600 text-white hover:bg-amber-700 transition-colors cursor-pointer shrink-0"
                      >
                        Edit
                      </button>
                    </div>
                    ...
                    <div className="p-2 rounded-lg bg-white border border-amber-200 text-slate-800 text-[11px] sm:text-xs">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-4 border-t border-slate-200">
            <button
              onClick={onBackToDraft}
              className="h-10 px-4 rounded-xl border border-slate-200 bg-white text-slate-800 text-xs font-bold transition-colors flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-50 w-full sm:w-auto"
            >
              <ArrowLeft className="w-4 h-4 text-indigo-600 shrink-0" />
              <span>Continue Editing</span>
            </button>

            <div className="grid grid-cols-3 gap-1.5 sm:gap-2.5 w-full sm:w-auto sm:flex sm:items-center">
              <button
                onClick={handleCopyText}
                className="h-10 px-1 sm:px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-[10px] sm:text-xs font-bold transition-colors flex items-center justify-center gap-1 sm:gap-1.5 cursor-pointer"
              >
              ...
              <button
                onClick={onSave}
                className="h-10 px-1 sm:px-5 rounded-xl bg-slate-900 hover:bg-black text-white text-[10px] sm:text-xs font-bold shadow-md transition-all flex items-center justify-center gap-1 sm:gap-1.5 cursor-pointer"
              >
                <Save className="w-3.5 h-3.5 shrink-0" />
                <span className="truncate">Save State</span>
              </button>
            </div>
          </div>
        </div>
