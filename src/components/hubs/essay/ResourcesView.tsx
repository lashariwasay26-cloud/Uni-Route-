...
export const ResourcesView: React.FC<ResourcesViewProps> = ({
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 py-4 animate-in fade-in-50">
      {/* Top Navigation Bar */}
      <div className="flex flex-wrap items-center justify-between gap-4 pb-2 border-b border-slate-200/80">
        <div className="flex items-center gap-2">
          ...
          {viewMode === 'detail' && (
            <button
              ...
              className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-all cursor-pointer"
            >
        ...
        {onStartWriting && (
          <button
            ...
            className="px-5 py-2.5 rounded-2xl bg-slate-900 hover:bg-black text-white font-bold text-xs shadow-md transition-all cursor-pointer flex items-center gap-2 active:scale-95"
          >
      ...
      {viewMode === 'grid' ? (
        <div className="space-y-6">
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full bg-white border border-slate-200 rounded-2xl pl-9 pr-4 py-2.5 text-xs text-slate-950 placeholder-slate-400 focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600/20 transition-all shadow-xs"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {filteredArticles.map((art) => (
              <div
                ...
                className="bg-white border border-slate-200/80 hover:border-indigo-500/80 rounded-[24px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-md transition-all cursor-pointer flex flex-col justify-between group space-y-4"
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-indigo-700 bg-indigo-50 px-2.5 py-1 rounded-lg">
                      {art.category}
                    </span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200/80 rounded-[28px] p-6 sm:p-8 space-y-8 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
            {/* Header Header */}
            <div className="space-y-3 pb-6 border-b border-slate-100">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="text-xs font-extrabold uppercase tracking-wider text-indigo-700 bg-indigo-50 px-3 py-1 rounded-xl border border-indigo-100">
              ...
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed italic bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
              ...
              {currentArticle.content.map((sec, idx) => (
                <div key={idx} className="space-y-3">
                  ...
                  {(sec.exampleWeak || sec.exampleStrong) && (
                    <div className="ml-8 my-3 grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                      {sec.exampleWeak && (
                        <div className="p-3.5 rounded-xl bg-rose-50/80 border border-rose-200/80 space-y-1">
                          <span className="font-extrabold text-rose-800 uppercase text-[10px] block">
                            Weak Approach
                          </span>
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-r from-indigo-900 to-indigo-950 text-white flex flex-wrap items-center justify-between gap-4 shadow-md">
              ...
              {onStartWriting && (
                <button
                  onClick={onStartWriting}
                  className="px-5 py-2.5 rounded-xl bg-white text-indigo-950 font-bold text-xs hover:bg-indigo-50 transition-colors cursor-pointer shrink-0 shadow-xs active:scale-95"
                >
                  Open Essay Builder →
                </button>
              )}
