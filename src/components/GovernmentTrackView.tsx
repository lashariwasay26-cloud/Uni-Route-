...
export const GovernmentTrackView: React.FC<GovernmentTrackViewProps> = ({
  ...
  return (
    <div className="relative overflow-x-hidden w-full min-h-[500px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 pb-4 sm:mb-8 sm:pb-6 border-b border-slate-200/80">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider text-indigo-700">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-3 sm:p-5 shadow-xs mb-4 sm:mb-8 space-y-3 sm:space-y-4">
        
        {/* Region Tabs */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pb-3 border-b border-slate-100">
          <div className="flex flex-row items-center gap-1.5 w-full lg:w-auto min-w-0">
            <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider shrink-0">Region:</span>
            <div className="flex items-center p-1 rounded-xl bg-slate-100 border border-slate-200/60 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden whitespace-nowrap w-full">
              <button
                ...
                className={`px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-bold transition-all shrink-0 cursor-pointer ${
                  selectedRegion === 'All'
          ...
          <div className="text-[10px] sm:text-xs text-slate-500 font-medium shrink-0 self-start lg:self-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center">
          ...
          <div className="relative flex-1">
            <Search className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-4.5 sm:h-4.5 text-slate-400" />
            <input
              type="text"
              placeholder="Search title, government, or country..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 sm:pl-11 pr-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl border border-slate-200 bg-slate-50/50 text-slate-900 text-xs sm:text-sm placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:bg-white transition-all"
            />
          </div>

          {/* Filter Chips Container */}
          <div className="flex items-center gap-1.5 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden whitespace-nowrap w-full lg:w-auto">
            <span className="text-[10px] sm:text-xs font-bold text-slate-400 mr-1 sm:mr-2 uppercase tracking-wider shrink-0">Level:</span>
            {allDegrees.map(deg => (
              <button
                key={deg}
      ...
      {filteredScholarships.length === 0 ? (
        <div className="text-center py-16 bg-white border border-slate-200/80 rounded-2xl max-w-xl mx-auto px-6">
        ...
        <>
            ...
            {visibleScholarships.map(item => {
              ...
              return (
                <div
                  key={item.id}
                  id={`gov-card-${item.id}`}
                  className="bg-white border border-slate-200/90 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xs hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 flex flex-col justify-between group hover:border-indigo-300 animate-fadeIn"
                >
                  <div>
                    
                    {/* Top Bar: Country Flag & Sponsor Info */}
                    <div className="flex flex-row items-center justify-between gap-3 mb-3 sm:mb-4 min-w-0">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className="text-2xl p-1.5 sm:text-3xl sm:p-2 rounded-xl sm:rounded-2xl bg-slate-50 border border-slate-100 shrink-0">
                          {item.flag}
                        </span>
                        <div className="min-w-0">
                          <span className="text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider text-indigo-600 block leading-tight break-words">
                            {item.sponsorGovernment}
                          </span>
                          <span className="text-[11px] text-slate-500 font-semibold mt-0.5 block leading-none">
                    ...
                    {item.currentStatus && (
                      <div className="mb-3">
                        <span className={`inline-block px-2.5 py-1 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-wider border leading-snug break-words max-w-full ${
                    ...
                    {/* Program Title */}
                    <h2 className="text-base sm:text-xl lg:text-2xl font-black text-slate-950 tracking-tight leading-snug mb-2 sm:mb-3 group-hover:text-indigo-600 transition-colors">
                      {item.programTitle}
                    </h2>

                    {/* Description */}
                    <p className="text-slate-600 text-[11px] sm:text-xs lg:text-sm leading-relaxed mb-4 sm:mb-6">
                      {item.description}
                    </p>

                    {/* Financial Package Banner */}
                    <div className="p-3 sm:p-4 bg-slate-50/80 rounded-xl sm:rounded-2xl border border-slate-100 mb-4 sm:mb-6 space-y-1.5 sm:space-y-2">
                      <div className="flex items-center justify-between text-xs gap-2">
                        <span className="text-slate-400 font-semibold uppercase tracking-wider text-[9px] sm:text-[10px] shrink-0">
                          FINANCIAL PACKAGE
                        </span>
                        <span className="font-black text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100 text-[9px] sm:text-[10px] truncate">
                          {item.fundingType || 'Fully Funded'}
                        </span>
                      </div>
                      <div className="text-[11px] sm:text-xs font-bold text-slate-800 flex items-start sm:items-center gap-1.5">
                        <DollarSign className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-600 shrink-0 mt-0.5 sm:mt-0" />
                        <span className="leading-snug">{item.stipendMonthly}</span>
                      </div>
                      <div className="text-[9px] sm:text-[10px] text-slate-500 flex flex-wrap items-center gap-1 sm:gap-1.5 pt-1.5 sm:pt-2 border-t border-slate-100/85">
                    ...
                    {/* Key Highlights Grid */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-4 sm:mb-6 text-[11px] sm:text-xs">
                      <div className="p-2.5 sm:p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                        <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 block uppercase">Work Experience</span>
                        <strong className="text-slate-900 font-bold block mt-0.5 truncate text-[11px] sm:text-xs" title={item.workExperienceRequired}>
                          {item.workExperienceRequired}
                        </strong>
                      </div>

                      <div className="p-2.5 sm:p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                        <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 block uppercase">Application Route</span>
                        <strong className="text-slate-900 font-bold block mt-0.5 truncate text-[11px] sm:text-xs" title={item.applicationRoute}>
                          {item.applicationRoute}
                        </strong>
                      </div>

                      <div className="p-2.5 sm:p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                        <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 block uppercase">HEC Nomination</span>
                        <strong className="text-slate-900 font-bold block mt-0.5 truncate text-[11px] sm:text-xs" title={item.hecNominationRequired ? 'Mandatory (HEC Portal)' : 'Direct / Not Required'}>
                          {item.hecNominationRequired ? 'Mandatory' : 'Direct'}
                        </strong>
                      </div>

                      <div className="p-2.5 sm:p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                        <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 block uppercase">Return Obligation</span>
                        <strong className="text-slate-900 font-bold block mt-0.5 truncate text-[11px] sm:text-xs" title={item.bondRequirement}>
                  ...
                  {/* Footer Action Bar */}
                  <div className="pt-3.5 sm:pt-5 border-t border-slate-100 flex flex-row items-center justify-between gap-2.5 sm:gap-3">
                    <div className="flex items-center gap-2 text-slate-500 text-[10px] sm:text-xs font-semibold">
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 h-9 sm:h-10 px-4 sm:px-5 rounded-lg sm:rounded-xl bg-slate-900 hover:bg-indigo-600 text-white text-[11px] sm:text-xs font-bold transition-all shadow-xs group-hover:bg-indigo-600 cursor-pointer"
                    >
          ...
          {filteredScholarships.length > visibleCount && (
            <div className="text-center pt-8">
              ...
              >
                <span>Show More ({filteredScholarships.length - visibleCount} remaining)</span>
